...
for i = 1:length(M_values)
    ...
    % MPSK
    Pe_mpsk = erfc(sqrt(2 * Eb_N0 * sin(pi/M)^2));
    Ps_mpsk(i, :) = 1 - (1 - Pe_mpsk).^log2(M);
    
    % MPAM
    Pe_mpam = erfc(sqrt(3 * log2(M) * Eb_N0 / (M^2 - 1)));
    Ps_mpam(i, :) = 1 - (1 - Pe_mpam).^log2(M);
    
    % MQAM
    Pe_mqam = 2 * (1 - 1/sqrt(M)) * erfc(sqrt(3 * log2(M) * Eb_N0 / (2 * (M - 1))));
    Ps_mqam(i, :) = 1 - (1 - Pe_mqam).^log2(M);
    
    % MFSK
    Ps_mfsk(i, :) = 2 * erfc(sqrt((6 * log2(M)) / (M^2 - 1) * Eb_N0));
    
    % Non-coherent MFSK
    Ps_mfsk_noncoherent(i, :) = 2 * erfc(sqrt((3 * log2(M)) / (2 * (M^2 - 1)) * Eb_N0));
end
% Plotting
